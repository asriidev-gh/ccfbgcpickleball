...
import { applyRecentEndCourtGuard } from "@/lib/recent-ended-court";
import type {
...
export async function fetchOperatorQueue(
  ...
  const data = applyRecentEndCourtGuard(
    gameId,
    (await fetchOperatorGame(gameId, "queue")) as OperatorQueuePayload,
  );
