...
import { cancelCourtAssignment } from "@/lib/queue-engine";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await cancelCourtAssignment(payload);

    return NextResponse.json({
      message: `Court ${payload.courtNumber} assignment cancelled. Players returned to the queue.`,
    });
  } catch (error) {
    return NextResponse.json(
      {
        message:
          error instanceof Error ? error.message : "Failed to cancel court assignment.",
      },
      { status: 400 },
    );
  }
