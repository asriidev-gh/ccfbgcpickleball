...
/** Auto-collapse balanced matchup details so Next on Court gets more room. */
const BALANCED_AUTO_COLLAPSE_MS = 3000;
...
export function NextCourtMatchAnalysis({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (!skipAutoCollapseRef.current) {
        setBalancedCollapsed(true);
      }
    }, BALANCED_AUTO_COLLAPSE_MS);
  ...
  const toggleBalancedCollapsed = () => {
    setBalancedCollapsed((current) => {
      const next = !current;
      if (!next) {
        // User expanded — don't auto-collapse again for this foursome.
  ...
  const collapseToggle =
    isBalanced ? (
      <Button
        ...
        variant="ghost"
        size="icon"
        className="next-court-analysis__collapse-btn"
        aria-expanded={!balancedCollapsed}
        aria-label={balancedCollapsed ? "Expand matchup check" : "Collapse matchup check"}
        onClick={toggleBalancedCollapsed}
      >
        <ChevronDown
          className={cn(
            "h-3.5 w-3.5 transition-transform",
            !balancedCollapsed && "rotate-180",
          )}
          aria-hidden
        />
      </Button>
  ...
  const header = (
    <div className="next-court-analysis__header">
      <div className="next-court-analysis__header-start">
        <p className="next-court-analysis__label">Matchup check</p>
        {isBalanced && balancedCollapsed ? (
          <span className="next-court-analysis__collapsed-badge">
            <CheckCircle2 className="h-3 w-3 text-emerald-500" aria-hidden />
            Balanced
          </span>
        ) : null}
      </div>
      <div className="next-court-analysis__header-actions">
        {helpButton}
        {collapseToggle}
      </div>
    </div>
  ...
  const showDetails = !isBalanced || !balancedCollapsed;

  return (
    <section
      className={cn(
        "queue-next-up-analysis",
        isBalanced && balancedCollapsed && "queue-next-up-analysis--collapsed",
        className,
      )}
